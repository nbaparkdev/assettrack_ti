...
func (r *UserRepository) GetByID(id uint) (*models.User, error) {
	...
	if err := r.db.Preload("Departamento").Preload("Localizacao").Preload("Gestor").First(&user, id).Error; err != nil {
		return nil, err
	}
	return &user, nil
}
...
func (r *UserRepository) GetByEmail(email string) (*models.User, error) {
	var user models.User
	if err := r.db.Preload("Departamento").Preload("Localizacao").Preload("Gestor").Where("email = ?", email).First(&user).Error; err != nil {
		return nil, err
	}
	return &user, nil
}
...
func (r *UserRepository) GetByQRToken(token string) (*models.User, error) {
	var user models.User
	if err := r.db.Preload("Departamento").Preload("Localizacao").Preload("Gestor").Where("qr_token = ?", token).First(&user).Error; err != nil {
		return nil, err
	}
	return &user, nil
}
...
func (r *UserRepository) GetMulti(skip, limit int) ([]models.User, error) {
	...
	if err := r.db.Preload("Departamento").Preload("Localizacao").Preload("Gestor").Offset(skip).Limit(limit).Find(&users).Error; err != nil {
		return nil, err
	}
	return users, nil
}
